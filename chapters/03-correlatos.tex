...
Desta revisão, temos que o \textit{FragCubing} continua sendo um algoritmo robusto para a computação do cubo, com suas técnicas de índice invertido sendo utilizadas e ainda obtendo resultados adequados~\cite{silva:2015:abordagensParaCubo}.

{\color{red}
LI; HAN e GONZALEZ (2004) ilustram o impacto exponencial no consumo de memória nas diferentes abordagens de computação de cubos de dados usando apenas 12 dimensões.
Nota-se que há uma clara saturação quando cubos com 20, 50 ou 100 dimensões são computados utilizando abordagens de cubos completos, cubos DWARF, MCG, cubos fechados ou quocientes (BRAHMI, 2012; RUGGIERI; PEDRESCHI; TURINI, 2010; LIMA; HIRATA, 2011; XIN; et al., 2006; SISMANIS, 2002).
}
...
% Falar do Frag especialmente e dar uma overview de como ele funciona e quais são as suas fraquezas

Índices invertidos são mais adequados quando a relação possui um elevado número de tuplas e dimensões com alta cardinalidade~\cite{silvaQCubeEfficientIntegration2013}, contudo técnicas como as adotadas por abordagens como \textit{Frag-Cubing} degradam o consumo de memória à medida que o número de tuplas cresce, pois consequentemente nestes cenários, o tamanho do índice invertido cresce substancialmente.

[Exemplo do Rodrigo ou não?]
...
Em nosso exemplo, iTb 4 com dois TIDs é a menor lista.
...
Listas de TIDs pequenas permitem que consultas sejam respondidas rapidamente, portanto relações com baixo skew e alta cardinalidade são mais adequadas de serem computadas pela abordagem Frag-Cubing.
Skew é o grau de uniformidade dos valores de atributos numa relação. Skew zero indica relação com valores de atributos uniformemente distribuídos e quanto maior o skew menos uniformemente distribuída a relação se encontra.
